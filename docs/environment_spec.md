...
## **CodeIgniter4 Tasksコマンド**
CodeIgniter4のTasks機能を利用して、バッチ処理やコマンドラインからの操作を実行します。
プロジェクトの`src`ディレクトリで以下のコマンドを実行してください。

### **利用可能なタスクの一覧表示**
```bash
php spark tasks:list
```
実行可能なすべてのタスクのリストが表示されます。

### **ファイルインポートタスクの実行**
```bash
php spark tasks:process_imports
```
このコマンドは、`import_tasks`テーブルに登録された未処理のインポートタスクを順次実行します。
Web画面からアップロードされたExcelファイルは、このコマンドによって非同期でデータベースに取り込まれます。
タスクスケジューラに登録し、定期的に実行することを推奨します。
